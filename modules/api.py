...
def create_app(config) -> FastAPI:
    ...
    async def search(
        ...
        filter_type: Optional[str] = Query(
            None, description="Filter type (none/canny)"
        ),
        match_method: Optional[str] = Query(None, description="Match method"),
        threshold: Optional[float] = Query(
            None, description="Match threshold (0.0-1.0)"
        ),
        ...
        offsetx: Optional[int] = Query(
            None, description="Offset X to add to result coordinates"
        ),
        offsety: Optional[int] = Query(
            None, description="Offset Y to add to result coordinates"
        ),
        waitForRecheck: Optional[float] = Query(
            0.0, description="Wait time for recheck"
        ),
        ...
        try:
            ...
            if "*" in img or "?" in img:
                ...
                if not template_paths:
                    raise HTTPException(
                        status_code=404, detail=f"No files match pattern: {img}"
                    )
            else:
                if not os.path.exists(img):
                    raise HTTPException(
                        status_code=404, detail=f"Template file not found: {img}"
                    )
                template_paths = [img]
            ...
            if filter_type:
                ...
                search_params["filter_type"] = server_config.get(
            ...
            if match_method:
                ...
                search_params["match_method"] = server_config.get(
            ...
            if search_params["filter_type"] == "canny":
                if canny_t1 is not None:
                    ...
                    search_params["canny_t1"] = server_config.get(
                        "default_canny_t1", 50
                    )
                ...
                if canny_t2 is not None:
                    ...
                    search_params["canny_t2"] = server_config.get(
            ...
            # Get current frame from stream
            frame, frame_timestamp, frame_dimensions = (
                app.state.stream_reader.get_frame()
            )
            ...
            for template_path in template_paths:
                try:
                    # Perform template matching
                    match_result = app.state.image_processor.match_template(
                        frame, template_path, search_params
                    )

                    # If found or this is the last template, use this result
                    if (
                        match_result.get("found", False)
                        or template_path == template_paths[-1]
                    ):
                        result = match_result
                        break
