...
PROCESS_START_TIMEOUT = 5.0  # seconds
...
def get_instance_status(port: int) -> Dict[str, Any]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.warning(f"Instance on port {port} returned status code {response.status_code}")
            return {
                "port": port,
                "status": "error",
            ...
            while time.time() - start_time < PROCESS_START_TIMEOUT:
                if is_port_in_use(port):
                    ...
                    if status["status"] == "running":
                        logger.info(f"Server instance on port {port} started successfully")
                        return {
                            "port": port,
                            "status": "started",
                            "pid": process.pid,
                        ...
                        logger.info(f"Server instance on port {port} started but API not ready yet")
                        # Keep waiting
                    else:
            ...
            if conn.laddr.port == port and conn.status == 'LISTEN':
                ...
                while time.time() - start_time < max_wait:
                    ...
                    try:
                        ...
                        if is_port_in_use(port):
                            logger.warning(f"Process ended but port {port} still in use")
                            break
                        else:
                ...
                logger.warning(f"Timeout waiting for process {pid} to terminate. Killing...")
...
def load_multi_config(config_path: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if not isinstance(config, list):
            logger.error(f"Invalid configuration format: {config_path} must contain a list")
            return []
        ...
        for idx, item in enumerate(config):
            if not isinstance(item, dict):
                logger.error(f"Invalid configuration at index {idx}: must be a dictionary")
                continue
        ...
        logger.info(f"Loaded {len(valid_configs)} valid instance configurations from {config_path}")
...
def main():
    ...
    start_parser.add_argument(
        "--port",
        type=int,
        help="Start a specific instance by port"
    )
    start_parser.add_argument(
        "--mjpeg-url",
    ...
    stop_parser.add_argument(
        "--port",
        type=int,
        help="Stop a specific instance by port"
    )
    
    # Status command
    status_parser = subparsers.add_parser("status", help="Get status of server instances")
    status_parser.add_argument(
        "--port",
        type=int,
        help="Get status of a specific instance by port"
    )
    
    # Update command
    update_parser = subparsers.add_parser("update", help="Update server instance configuration")
    update_parser.add_argument(
        "--port",
        type=int,
        required=True,
        help="Port of the instance to update"
    )
    update_parser.add_argument(
        "--new-mjpeg-url",
        required=True,
        help="New MJPEG URL for the instance"
    )
    
    # Create command
    create_parser = subparsers.add_parser("create", help="Create a new server instance configuration")
    create_parser.add_argument(
        "--port",
        type=int,
        required=True,
        help="Port for the new instance"
    )
    create_parser.add_argument(
        "--mjpeg-url",
        required=True,
        help="MJPEG URL for the new instance"
    )
    
    # Delete command
    delete_parser = subparsers.add_parser("delete", help="Delete a server instance configuration")
    delete_parser.add_argument(
        "--port",
        type=int,
        required=True,
        help="Port of the instance to delete"
    )
    ...
    if args.command == "start":
        ...
        if args.port:
            ...
            instance_config = next((item for item in multi_config if item["port"] == args.port), None)
            ...
            if instance_config:
                ...
                if args.mjpeg_url:
                    ...
                    logger.info(f"Overriding MJPEG URL for port {args.port}: {mjpeg_url}")
            elif args.mjpeg_url:
                # Instance not in config but MJPEG URL specified
                mjpeg_url = args.mjpeg_url
                logger.info(f"Starting new instance on port {args.port} with MJPEG URL: {mjpeg_url}")
            else:
                # Instance not in config and no MJPEG URL specified
                logger.error(f"No configuration found for port {args.port} and no MJPEG URL specified")
                return 1
            ...
            if result["status"] in ["started", "already_running"] and not instance_config:
                multi_config.append({
                    "port": args.port,
                    "mjpeg_url": mjpeg_url
                })
        ...
        instance_config = next((item for item in multi_config if item["port"] == args.port), None)
        if instance_config:
            logger.error(f"Instance with port {args.port} already exists in configuration")
            return 1
