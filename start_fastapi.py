...
def parse_arguments():
    ...
    # Required arguments
    parser.add_argument(
        "--mjpeg-url", default=DEFAULT_MJPEG_URL, help="URL of the MJPEG stream"
    )

    # Server configuration
    parser.add_argument("--host", default=DEFAULT_HOST, help="Host to bind the server")
    parser.add_argument(
        "--port", type=int, default=DEFAULT_PORT, help="Port to bind the server"
    )
    ...
    parser.add_argument("--log-file", default="server.log", help="Log file path")
    ...
    parser.add_argument(
        "--enable-debug-saving", action="store_true", help="Enable saving debug images"
    )
    parser.add_argument(
        "--max-debug-files",
        type=int,
...
def main():
    ...
    try:
        ...
        try:
            ...
            if (
            ...
            ):  # Only apply if the port is still the default value
                ...
                if len(ip_parts) == 4:
                    ...
                    logger.info(
                        f"Dynamically calculated port based on MJPEG URL: {dynamic_port}"
                    )
